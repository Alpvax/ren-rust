...
use super::{
    extensions::SyntaxIterator, macro_impl::create_ast_enum, FromSyntaxElement, SyntaxToken, ToHIR,
};
...
impl ToHIR for TFun {
    ...
    fn to_higher_ast(&self, line_lookup: &line_col::LineColLookup) -> Self::HIRType {
        HigherType::fun(self.arg().map(|t| t.to_higher_ast(line_lookup)), self.ret().map(|t| t.to_higher_ast(line_lookup)).unwrap())
    }
...
impl ToHIR for TSum {
    ...
    fn to_higher_ast(&self, line_lookup: &line_col::LineColLookup) -> Self::HIRType {
        HigherType::sum(self.parts().map(|(s, v)| (s, v.into_iter().map(|t| t.to_higher_ast(line_lookup)))))
    }
